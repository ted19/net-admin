...
#pragma warning restore CS8602 // null 가능 참조에 대한 역참조입니다.
            ...
            if (userData.users_group_idx == 0)
            {
                usersGroupMenuData = _context.Users_group_menu.Where(x => x.admin_idx == userData.idx && x.controller == controller && x.action == action).FirstOrDefault();
            }
        ...
        public class LogActionAttribute : Attribute, IActionFilter
        {
            ...
            {
                _division = ((ControllerBase)context.Controller).ViewBag.division;
                _menu = ((ControllerBase)context.Controller).ViewBag.menu;
                _item = ((ControllerBase)context.Controller).ViewBag.item;
                item_sub = ((ControllerBase)context.Controller).ViewBag.item_sub;
                _count = ((ControllerBase)context.Controller).ViewBag.count;
                before = ((ControllerBase)context.Controller).ViewBag.before;
                after = ((ControllerBase)context.Controller).ViewBag.after;
                _reason = ((ControllerBase)context.Controller).ViewBag.reason;
                _action = ((ControllerBase)context.Controller).ViewBag.action;
                _target_id = ((ControllerBase)context.Controller).ViewBag.target_id;
...
#pragma warning restore CS8602 // null 가능 참조에 대한 역참조입니다.
                ...
                var afterData = AfterData(controller, action, requestBody);
...
#pragma warning restore CS8600 // null 리터럴 또는 가능한 null 값을 null을 허용하지 않는 형식으로 변환하는 중입니다.
                ...
                Dictionary<string, string> beforeList = new Dictionary<string, string>();
                Dictionary<string, string> afterList = new Dictionary<string, string>();

                if (_action == "Edit" || _action == "Create" || _action == "Delete")
                {
#pragma warning disable CS8604 // 가능한 null 참조 인수입니다.
                    JObject beforeDataFields = JObject.Parse(beforeData);
#pragma warning restore CS8604 // 가능한 null 참조 인수입니다.

                    foreach (JProperty x in (JToken)beforeDataFields)
                    {
                        string key = x.Name;
                        string value = x.Value.ToString();

                        beforeList.Add(key, value);
                    }

                    JObject afterDataFields = JObject.Parse(afterData);

                    foreach (JProperty x in (JToken)afterDataFields)
                    {
                        string key = x.Name;
                        string value = x.Value.ToString();

                        afterList.Add(key, value);
                    }

                    if (_action == "Edit")
                    {
                        foreach (KeyValuePair<string, string> bl in beforeList)
                        {
                            if (afterList[bl.Key].Equals(bl.Value) == true)
                            {
                                _logger.LogInformation("aaa Key: {0}, Value: {1}", bl.Key, bl.Value);

                            }
                            else if (afterList[bl.Key].Equals(bl.Value) == false)
                            {
                                _logger.LogInformation("bbb Key: {0}, Value: {1}", bl.Key, bl.Value);
                                changeList.Add(bl.Key, bl.Value);
                            }
                        }
                    }
                }
                ...
                if (_context.Users != null)
                {
                    ...
                    if (userData != null && _context.Admin_log != null)
                    {
                        if (_action == "Edit")
                        {
                            foreach (KeyValuePair<string, string> cl in changeList)
                            {
                                if (cl.Key == "create_date")
                                    continue;

                                _logger.LogInformation("ccc Key: {0}, Value: {1}", cl.Key, cl.Value);

                                if (String.IsNullOrEmpty(item_sub) == true)
                                {
                                    if (_item == "Authority group edit" || _item == "Group menu edit")
                                    {
                                        _item_sub = afterList["name"] + " > " + cl.Key;
                                    }
                                    else
                                    {
                                        _item_sub = cl.Key;
                                ...
                                if (String.IsNullOrEmpty(before) == true)
                                {
                                    _before = beforeList[cl.Key];
                                }
                                else
                                {
                                    _before = before;
                                ...
                                if (String.IsNullOrEmpty(after) == true)
                                {
                                    _after = afterList[cl.Key];
                                }
                                else
                                {
                                    _after = after;
                                ...
                                admin_log.target_id = _target_id;
                                ...
                                admin_log.after_data = afterData;
                                admin_log.division = _division;
                                admin_log.menu = _menu;
                                admin_log.item = _item;
                                admin_log.item_sub = _item_sub;
                                admin_log.count = _count;
                                admin_log.before = _before;
                                admin_log.after = _after;
                                admin_log.reason = _reason;
                        ...
                        else
                        {
                            ...
                            if (_action == "Create")
                            {
                                if (_item == "Authority group create" || _item == "Group menu create")
                                {
                                    _item_sub = afterList["name"];
                                }
                            }
                            else if (_action == "Delete")
                            {
                                if (_item == "Authority group delete" || _item == "Group menu delete")
                                {
                                    _item_sub = beforeList["name"];
                                }
                            }
