...
namespace webAdmin.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([Bind("idx,user_id,user_pw,user_pw_confirm,name,dept,email,status,pw_update_date,pw_error_count,users_group_idx,reason")] User user)
        {
            ViewBag.division = "Change";
            ViewBag.menu = "Admin list";
            ViewBag.item = "Admin create";
            ViewBag.item_sub = "";
            ViewBag.reason = user.reason;
            ...
            if (ModelState.IsValid)
            {
                ...
                login_log.division = "System login";
				login_log.pw_error_count = user.pw_error_count;
        ...
        public async Task<IActionResult> Edit(int? id, [Bind("idx,user_id,user_pw,user_pw_confirm,name,dept,email,status,pw_error_count,users_group_idx,pw_update_date,reason")] Admin admin)
        {
			...
			ViewBag.division = "Change";
			ViewBag.menu = "Admin list";
			ViewBag.item = "Admin edit";
			ViewBag.item_sub = "";
            ViewBag.reason = admin.reason;
            ViewBag.action = "Edit";
            ...
            if (admin.users_group_idx > 0)
            {
                var removeUsersGroupMenu = _context.Users_group_menu.Where(x => x.admin_idx == id).ToList();
                _context.Users_group_menu.RemoveRange(removeUsersGroupMenu);
                await _context.SaveChangesAsync();
            }
        ...
        [TypeFilter(typeof(LogActionAttribute))]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete(int? id, [Bind("reason")] Admin admin)
        {
            ViewBag.division = "Change";
            ViewBag.menu = "Admin list";
            ViewBag.item = "Admin delete";
            ViewBag.item_sub = "";
            ViewBag.reason = admin.reason;
